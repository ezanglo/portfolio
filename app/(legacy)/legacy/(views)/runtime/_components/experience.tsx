...
function ExperienceRow({ experience }: { experience: NormalizedExperience }) {
  return (
    <div className="grid gap-2 border-t border-view-border py-6 first:border-t-0 first:pt-0 sm:grid-cols-[170px_1fr] sm:gap-6">
      <div className="font-mono text-[12.5px] text-view-fg-subtle">{experience.yearRange}</div>
      <div>
        <h3 className="text-base font-semibold text-view-fg">{experience.title}</h3>
        <p className="font-mono text-sm text-view-accent">
...
export default function RuntimeExperience({
  experience,
  copy,
}: {
